...
// filter values
int16_t last_raw_x[3] = {0, 0, 0};
int16_t last_filtered_x[3] = {0, 0, 0};
int16_t last_raw_y[3] = {0, 0, 0};
int16_t last_filtered_y[3] = {0, 0, 0};
...
int16_t update_touch_x(void) {
   last_raw_x[2] = last_raw_x[1];
   last_raw_x[1] = last_raw_x[0];
   int i;
   for(i = 0; i < 50; i++) {
       last_raw_x[0] = touch_read();
       if(abs(last_raw_x[1] - last_raw_x[0]) < 150) {
           break;
       }
   }
   int16_t new_filtered_x = (
           ((0.0675) * last_raw_x[0] +
           (0.1349) * last_raw_x[1] + 
           (0.0675) * last_raw_x[2]) - 
           ((-1.1430) * last_filtered_x[0] +
           (0.4128) * last_filtered_x[1])
   );
           
   last_filtered_x[2] = last_filtered_x[1];
   last_filtered_x[1] = last_filtered_x[0];
   last_filtered_x[0] = new_filtered_x;
   
   return new_filtered_x;
//    lcd_locate(0,3);
//    lcd_printf("(%d, %d)", last_filtered_x[1], last_filtered_y[1]);
   
}

int16_t update_touch_y(void) {
   
   last_raw_y[2] = last_raw_y[1];
   last_raw_y[1] = last_raw_y[0];
   int i;
   for(i = 0; i < 50; i++) {
       last_raw_y[0] = touch_read();
       if(abs(last_raw_y[1] - last_raw_y[0]) < 150) {
           break;
       }
   }
   int16_t new_filtered_y = (
           ((0.0675) * last_raw_y[0] +
           (0.1349) * last_raw_y[1] + 
           (0.0675) * last_raw_y[2]) - 
           ((-1.1430) * last_filtered_y[0] +
           (0.4128) * last_filtered_y[1])
   );
           
   last_filtered_y[2] = last_filtered_y[1];
   last_filtered_y[1] = last_filtered_y[0];
   last_filtered_y[0] = new_filtered_y;
   
   return new_filtered_y;
...
void __attribute__((__interrupt__)) _T1Interrupt(void) {
   ...
   if (mole_count == 0) {
       ...
       matched = 0;
       cumulative_count+=1;
//        lcd_printf("SENTTTTTTT");
//        drawBox(mole_x-2, mole_y-2, mole_x+2, mole_y+2, 1);
       //__delay_ms(2000);
       //lcd_clear();
   } 
   mole_count = (mole_count + 1) % 3;
   if(mole_count==0){
       mole_x = 0;
       mole_y = 0;
   }
   if (mole_count==0 || mole_count==1){
       uart2_send_8(mole_x);
//        __delay_ms(2);
       while(!uart2_recv(&receive));
       uart2_send_8(mole_y);
       receive=0;
   }
//    drawBox(mole_x-2, mole_y-2, mole_x+2, mole_y+2, 0);
...
//        touch_select_dim_nodelay(0);
       ...
       for(i = 0; i < 40; i++) {
           ...
           if(abs(update_ball_touch_pos_y - ball_touch_pos_y) < 100) {
              break;
           }
       }
...
//    }
   ...
   // timer1 for drawing mole , interrupt every 1s
   set_timer1(50000);
   ...
   // stop updating button values
   lcd_printf("timer three disabling")
   // disable timer2
   T2CONbits.TON = 0;
   lcd_clear();
